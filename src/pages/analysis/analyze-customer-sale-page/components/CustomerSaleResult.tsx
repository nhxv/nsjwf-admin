  ...
  return (
    <>
      <div className="my-4 flex items-center justify-end gap-2">
        ...
        <button className="rounded-btn flex bg-accent p-2 text-sm font-semibold hover:text-primary" onClick={onExportToCSV}>
      ...
      <div className="custom-card w-full">
        ...
        {customerSales.map((customerSale) => (
          <div key={customerSale.customerName} className="my-2 flex w-full justify-between rounded-lg bg-base-200 p-3 dark:bg-base-300">
            <div className="md:w-6/12">{customerSale.customerName}</div>
            <div className="md:w-5/12">{customerSale.boxCount}</div>
            <div className="hidden md:flex md:w-1/12">${customerSale.avgPrice}</div>
          </div>
        ))}
      </div>
    </>

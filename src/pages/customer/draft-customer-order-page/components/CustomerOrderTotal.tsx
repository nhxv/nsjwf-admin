...
interface ICustomerOrderTotalProps {
  control: Control<any>;
}
...
export default function CustomerOrderTotal({ control }: ICustomerOrderTotalProps) {
  const products = useWatch({ control, name: "products" }) ?? [];
  const total = products.length > 0 ? niceVisualDecimal(+products.reduce((prev, current) => prev + current.quantity * +current.price, 0)) : "0";

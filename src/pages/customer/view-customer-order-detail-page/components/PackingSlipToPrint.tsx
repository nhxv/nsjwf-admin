...
const MAX_LINES_PER_PAGE: number = 20;
...
export default function PackingSlipToPrint({ printRef, order }) {
  ...
  const renderPages = [];
  for (const [key, packingSlipPage] of SplitProductsIntoPages(order, columnNumber).entries()) {
    renderPages.push(
...
function SplitProductsIntoPages(order, columnCount: number = 2) {
  ...
  for (let i = 0; i < order.productCustomerOrders.length; i += MAX_LINES_PER_PAGE * columnCount) {
    // Sort by Cooler.
    const productCustomerOrders = [...order.productCustomerOrders].sort((productOrderA, productOrderB) => {
      ...
      <div className="flex flex-col">
        ...
        <div className="mb-2">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="block font-serif text-[0.625rem]">New San Jose Wholesale Foods, Inc.</h1>
            ...
            <div>
              <p className="text-xs">
                Invoice No.: <span className="font-semibold">{order.manual_code ? order.manual_code : order.code}</span>
              </p>
              <p className="text-xs">
                Ship at: <span className="font-semibold">{convertTimeToText(new Date(order.expected_at))}</span>
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex basis-0 gap-x-3">
          ...
          {Array.from({ length: columnCount }, (_, i) => i).map((ind) => (
            <div className="flex w-full justify-between border-b-4 border-black pb-1" key={`${ind}`}>
              <div className="flex">
        ...
        <div className={columnStyling}>
          ...
          <div>
            {productCustomerOrders.slice(i, i + MAX_LINES_PER_PAGE).map((productOrder) => (
              <div
          ...
          {/* 2nd column */}
          <div>
            {productCustomerOrders.slice(i + MAX_LINES_PER_PAGE, i + MAX_LINES_PER_PAGE * columnCount).map((productOrder) => (
              <div
                key={`${order.code}-${productOrder.product_name}-${productOrder.unit_code}`}
                className="flex w-full justify-between border-b border-black py-2">
                <div className="flex">

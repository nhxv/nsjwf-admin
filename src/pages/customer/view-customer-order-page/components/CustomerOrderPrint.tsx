    ...
    try {
      ...
      if (printerStatus.isReadyToPrint) {
        for (let i = 0; i < pallet.count; i++) {
          ...
          const customerName = `^FO100,100^ADN,36,20^FD${order.customerName}^FS`
          const date = `^FO150,150^ADN,36,20^FD${convertTime(new Date(order.expectedAt))}^FS`
          const page = `^FO200,200^ADN,36,20^FD${i+1}^FS`;

    ...
    <section className="min-h-screen">
      ...
      ${
        order.status === OrderStatus.PICKING
          ? "border-yellow-700 bg-yellow-100 text-yellow-700 dark:bg-yellow-900 dark:bg-opacity-10 dark:border-yellow-700"
          : ""
      }
      ${
        order.status === OrderStatus.CHECKING
          ? "border-neutral bg-base-100 dark:bg-base-200 dark:border-neutral"
          : ""
      }
      ${
        order.status === OrderStatus.SHIPPING
          ? "border-purple-700 bg-purple-100 text-purple-700 dark:bg-purple-900 dark:bg-opacity-10 dark:border-purple-700"
          : ""
      }
      ${
        order.status === OrderStatus.DELIVERED
          ? "border-sky-700 bg-sky-100 text-sky-700 dark:bg-sky-900 dark:bg-opacity-10 dark:border-sky-700"
          : ""
      }
      ${
        order.status === OrderStatus.COMPLETED
          ? "border-emerald-700 bg-emerald-100 text-emerald-700 dark:bg-emerald-900 dark:bg-opacity-10 dark:border-emerald-700"
          : ""
      }`}
          >

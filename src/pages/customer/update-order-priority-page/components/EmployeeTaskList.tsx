        ...
        {employees.map((employee) => (
          <Droppable key={employee.nickname} droppableId={employee.nickname}>
            {(provided, snapshot) => (
              ...
              className={`rounded-box p-2 w-full shadow-md border-2 bg-base-100 dark:bg-base-200 self-start ${snapshot.isDraggingOver ? "border-primary" : "border-base-100"}`}>
                  ...
                  return (
                    <Draggable key={order.code} draggableId={order.code} index={index} isDragDisabled={order.is_doing}>
                      {(provided, snapshot) => (
                        ...
                        ${order.is_doing ? "bg-red-900 text-white" : "bg-base-200 dark:bg-base-300"}
